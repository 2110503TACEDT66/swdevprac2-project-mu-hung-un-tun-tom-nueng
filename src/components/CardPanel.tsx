...
export default function CardPanel() {
    ...
    const ratingReducer = ( ratingList:Map<string, number>, action:{type:string, companyName:string, rating:number} )=>{
        switch(action.type) {
            case 'add' :{
    ...
    return (
        <div>
            ...
            <div className='w-full text-center p-[20px]'>
                {Array.from(ratingList).map(([company, rating]) => <div data-testid={company} key={company} onClick={ ()=>dispatchRating( {type:'remove' , companyName:company , rating:rating} ) }>
                {rating !== null ? `${company} : ${rating}` : null} </div> )}
            </div>
        </div>
    )
}